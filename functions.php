...
// ── Prix "À partir de" produits variables ──────────────
...
function chg_theme_setup() {
    ...
    add_theme_support( 'woocommerce', [ 'thumbnail_image_width' => 600, 'single_image_width' => 900, 'product_grid' => [ 'default_columns' => 3, 'default_rows' => 4, 'min_columns' => 1, 'max_columns' => 4 ] ] );
    add_theme_support( 'wc-product-gallery-zoom' );
    add_theme_support( 'wc-product-gallery-lightbox' );
    add_theme_support( 'wc-product-gallery-slider' );
    register_nav_menus( [ 'primary' => __( 'Menu Principal', 'chargeurie' ), 'footer_1' => __( 'Footer — Produits', 'chargeurie' ), 'footer_2' => __( 'Footer — Informations', 'chargeurie' ), 'footer_3' => __( 'Footer — Service Client', 'chargeurie' ) ] );
...
function chg_widgets_init() {
    $cols = [ 'footer-1' => 'Footer — Présentation', 'footer-2' => 'Footer — Produits', 'footer-3' => 'Footer — Informations', 'footer-4' => 'Footer — Service Client' ];
    foreach ( $cols as $id => $name ) {
        register_sidebar( [ 'name' => __( $name, 'chargeurie' ), 'id' => $id, 'before_widget' => '<div class="footer-widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h4 class="widget-title">', 'after_title' => '</h4>' ] );
    }
}
...
add_filter( 'woocommerce_account_menu_items', function( $items ) {
    return [ 'dashboard' => 'Tableau de bord', 'orders' => 'Mes commandes', 'edit-address' => 'Mes adresses', 'edit-account' => 'Mes informations', 'customer-logout' => 'Déconnexion' ];
} );
